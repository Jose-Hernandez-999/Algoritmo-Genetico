...
#VISUALIZACION
def visualizar_layout(poblacion):
    mejor = poblacion.mejor_genoma

    fig, (ax_prod, ax_rest) = plt.subplots(1, 2, figsize=(14, 6))

    # --- PRODUCCION ---
    for depto in mejor.deptos_produccion:
        esquina_x = depto.centroide_x - depto.ancho / 2
        esquina_y = depto.centroide_y - depto.alto / 2
        rect = patches.Rectangle(
            (esquina_x, esquina_y), depto.ancho, depto.alto,
            linewidth=1, edgecolor='black', facecolor='lightblue'
        )
        ax_prod.add_patch(rect)
        ax_prod.text(
            depto.centroide_x, depto.centroide_y, depto.codigo,
            ha='center', va='center', fontsize=9
        )

    ax_prod.set_xlim(0, Genoma.largo_produccion)
    ax_prod.set_ylim(0, Genoma.largo_produccion)
    ax_prod.set_aspect('equal')
    ax_prod.set_title(f'Produccion  |  MHC: {mejor.fitness:.2f}')
    ax_prod.set_xlabel('Ancho (m)')
    ax_prod.set_ylabel('Largo (m)')

    # --- RESTAURANTE ---
    for depto in mejor.deptos_restaurante:
        esquina_x = depto.centroide_x - depto.ancho / 2
        esquina_y = depto.centroide_y - depto.alto / 2
        rect = patches.Rectangle(
            (esquina_x, esquina_y), depto.ancho, depto.alto,
        ...
        ax_rest.add_patch(rect)
        ax_rest.text(
            depto.centroide_x, depto.centroide_y, depto.codigo,
            ha='center', va='center', fontsize=9
        )

    ax_rest.set_xlim(0, Genoma.largo_restaurante)
    ax_rest.set_ylim(0, Genoma.largo_restaurante)
    ax_rest.set_aspect('equal')
    ax_rest.set_title('Restaurante')
    ax_rest.set_xlabel('Ancho (m)')
    ax_rest.set_ylabel('Largo (m)')

    plt.tight_layout()
    plt.show()

visualizar_layout(poblacion)